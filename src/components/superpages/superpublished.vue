		...
		<div class="lost-main">
            <div v-for="(data,index) in myfoundList" class="lost-item" 
               ...
               >
                ...
                <p v-html="data.cover_desc"  class="lost_content">{{data.cover_desc}}</p>
	    ...
	    methods: {
			...
			deletemyfound:function(data){
				...
				deletemypublish(username,data.id).then(res =>{
					...
					if( response.code == 200){
						...
						//  this.$router.go(0)
						this.myfoundList  = []
						getmyfound(this.username,'Ariticle').then(res =>{
							let result = res.data;
							if( result.code == 200){
			...
			getData(){
				...
				getmyfound(this.username,'Ariticle').then(res =>{
					let result = res.data;
					if( result.code == 200){
						this.temp  = res.data.extra;
						this.temp  = this.temp.reverse();
						console.log(this.temp,11)
					}
				}).catch(function(error){
					console.log(error);
				});
				getmyfound(this.username,'Ariticle_today').then(res =>{
					let result = res.data;
					if( result.code == 200){
						this.myfoundList  = res.data.extra;
						this.myfoundList = this.myfoundList.concat(this.temp);
						// this.myfoundList  = this.myfoundList.reverse();
						console.log(this.myfoundList,1122)
					}
				}).catch(function(error){
					console.log(error);
				});
